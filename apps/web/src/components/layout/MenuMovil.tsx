...
/** Panel a pantalla completa sobre fondo tinta. Foco atrapado, Escape cierra, bloquea scroll. */
  ...
  return (
    ...
    >
      ...
      <nav aria-label="Menú móvil" className="flex flex-col px-lat-movil pt-6">
        ...
        {principales.map((servicioId) => (
          <Link
            key={servicioId}
            href={RUTA_SERVICIO[servicioId]}
            onClick={onCerrar}
        ...
        <div className="flex flex-wrap gap-x-5 gap-y-1 pt-2 pb-5 text-16">
          {secundarios.map((s) => (
            <Link
              ...
              onClick={onCerrar}
              className="py-[6px] text-sobre-tinta/70 no-underline"
            >
              {s.texto}
            </Link>
          ))}
        ...
        <div className="flex flex-col border-t border-sobre-tinta/[.16] pt-4">
          {[
            ...
            <Link
              ...
              href={enlace.href}
              onClick={onCerrar}
              className="text-20 font-semibold py-[10px] text-sobre-tinta no-underline"
            >
              {enlace.texto}
            </Link>
          ))}
      ...
      <div className="mt-auto p-lat-movil flex flex-col gap-3">
        ...
        {nap.whatsappHref ? (
          <Boton variante="contorno" sobreOscuro href={nap.whatsappHref} anchoCompleto>
            WhatsApp
          </Boton>
        ) : null}
      </div>
